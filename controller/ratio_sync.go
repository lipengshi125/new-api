	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			var pricingItems []struct {
				...
				CustomRatios         map[string]map[string]float64     `json:"custom_ratios"`
			}
			...
			customRatiosMap := make(map[string]map[string]map[string]float64)
			...
			for _, item := range pricingItems {
				...
				if item.CustomRatios != nil && len(item.CustomRatios) > 0 {
					customRatiosMap[item.ModelName] = item.CustomRatios
				}
			}
			...
			if len(customRatiosMap) > 0 {
				customAny := make(map[string]any, len(customRatiosMap))
				for k, v := range customRatiosMap {
					customAny[k] = v
				}
				converted["custom_ratios"] = customAny
			}
